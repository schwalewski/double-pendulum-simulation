...
This projects presenst a numerical solution to a classical physics problem using Python.
![Animation](pendulum.gif)

Physics background:

![Picture](pendulum.jpg)

To solve this problem we need to write system of equations describing our pendulum, let's use $\theta_1$ and $\theta_2$ as our general coordinates.
Now we can write the $XY$ coordinates od masses $m_1$ and $m_2$ using these angles:
$$
...
Lagrangian function is defined as $L = T - V$ where $T$ is kinetic energy and $V$ is potential energy.
Those energies can be written in terms of $\theta_1$ and $\theta_2$ as:
$$
...
$$
Now we have the system of second-order diffrential equations:
$$
